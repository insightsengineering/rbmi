...
Using the example described in the first section, let's say that we want to perform a delta-strategy where the delta applies to all imputed data after treatment discontinuation (regardless of the treatment arm). Assume that we want to fix `delta` proportional to the unadjusted observed mean change from baseline of the outcome value over all trial population. That is, we add `delta` to a missing data after treatment discontinuation at a given visit proportional to the observed mean change from baseline at that visit. Let's first determine the observed mean change from baseline at each visit:
