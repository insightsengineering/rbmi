...
tipp_frame <- expand.grid(
  delta_control = seq(-5, 15, by = 1),
  delta_intervention = seq(-5, 15, by = 1))
tipp_frame$pval_6 <- tipp_frame$trt_effect_6 <- NA

for (i in (1:nrow(tipp_frame))){
  # Create delta-adjustment 
  delta_control <- (dat_delta$group == "Control")*
    (dat_delta$is_missing)*
    (tipp_frame$delta_control[i])
  
  delta_interv <- (dat_delta$group == "Intervention")*
    (dat_delta$is_missing)*
    (tipp_frame$delta_intervention[i])
  
  dat_delta$delta <- delta_control + delta_interv
