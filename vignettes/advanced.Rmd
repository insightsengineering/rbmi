...
tmp <- as.data.frame(pool_obj_prop) %>%
    select(parameter, OR = est, OR.lci = lci, OR.uci = uci)
tmp
```

### Transforming pooled parameters

Some analysis functions estimate a parameter on a convenient model scale but
need to report it on another scale. This is possible with the `transform` argument 
to `analyse()`. Alternatively you can attach a transformation to the analysis
function with `attr(my_analysis, "transform")`. You can specify the transformation
directly as a list with elements `transform` and `derivative`, or use the convenience
function `use_transform(expression)`, where `expression` is a vectorized expression in `x`. 
The estimates are pooled on the original scale; `pool()` then stores the transformed estimate, confidence
limits, and delta-method standard error separately in `transformed_pars`.
Confidence limits are transformed directly rather than reconstructed from that
standard error. Transformations must be monotone over each pooled confidence
interval; `pool()` checks the supplied derivative and errors when it changes sign.
P-values are unchanged.

For example, the preceding logistic regression estimates a log odds ratio. We declare
odds-ratio reporting with the `exp` transformation. `print(pool_obj_prop)` and `as.data.frame(pool_obj_prop)`
now automatically use that scale, while the original log-odds-ratio result
remains in `pool_obj_prop$pars`:

```{r}
as.data.frame(pool_obj_prop)
as.data.frame(pool_obj_prop, scale = "original")
```

The same pattern can be used for other smooth, componentwise transformations.
...
Delta-adjustments are used to impute missing data under a missing not at random (MNAR) assumption. This reflects the belief that unobserved outcomes would have been systematically "worse" (or "better") than "comparable" observed outcomes. For an extensive discussion of delta-adjustment methods, we refer to @CroEtAlTutorial2020.
