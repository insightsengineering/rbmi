...
As an example, let's assume we want to use the copy  increments from reference strategy for the main analysis and explore the jump to reference imputation strategy as a sensitivity analysis. This could be implemented as follows:
...
draw_obj <- draws(
    ...
    vars = vars,
    method = method_condmean(type = "bootstrap", n_samples = 10)
    # Note that the chosen number of bootstrap samples of 10 would be much too low for 
    # reliable inference. 
)
...
strategy. To do this there are 3 things the user needs to do:
...
By default rbmi will analyse the data by fitting an ANCOVA model to the outcome from each visit separately,
and returning the "treatment effect" estimate as well as the corresponding least square means
