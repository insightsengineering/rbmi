...
!!! My apologies, the revised example creates an error currently. 

```{r}
compare_prop_lastvisit <- function(data,...){
  fit <- summary(glm(I(outcome>10)~group,family=binomial(),data=data[data[[visit]] == "visit_3", ]))
  res <- list( trt = list(est = fit$coefficients["groupB","Estimate"], se = fit$coefficients["groupB","Std. Error"], df = Inf) )
  return(res)
}
```
...
!!! The example below used the wording "the ICE occurred at" but in data_ICE we only know the first visit affected by the ICE.
...
For example lets say we have a subject whose first visit affected by the ICE was week 5 (i.e. visit v2) then 
