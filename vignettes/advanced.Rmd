...
The other event (ICE2) may be thought of as discontinuation from study treatment due to not study drug or condition related (SDCR) reasons. 
...
- Study drop-out at the study drug discontinuation visit occurrs with a probability of 50% leading to missing outcome data from that time point onward.
...
# Handling of observed post-ICE data in `rbmi` and imputation under CIR {#sec:postICEobs} 
...
`r ifelse(pool_obj_CIR$pars$trt_6$pval<0.001,"<0.001",formatC(pool_obj_CIR$pars$trt_6$pval,format="f",digits=3))`.
...
data_ice_JR$strategy[data_ice_JR$strategy=="CIR"] <- "JR"
...
data_ice_AVG$strategy[data_ice_AVG$strategy=="CIR"] <- "AVG"
...
# analyse
ana_obj_CIR6 <- analyse(impute_obj_CIR, vars = vars_an, fun = compare_change_lastvisit)
...
visit between the treatment groups with the baseline outcome as an additional covariate. This could then lead to the following basic analysis function:

```{r}
# Basic analysis function
compare_prop_lastvisit <- function(data, ...) {
    fit <- glm(I(change>10) ~ group + outcome_bl, family=binomial(), data = data, subset=(visit==6) )
    res <- list(
        trt = list(
            est = coef(fit)["groupIntervention"],
            se = sqrt(vcov(fit)["groupIntervention", "groupIntervention"]),
...
# analyse
ana_obj_prop <- analyse(impute_obj_CIR, vars = vars_an, fun = compare_prop_lastvisit)

# pool
pool_obj_prop <- pool(ana_obj_prop)
pool_obj_prop

# convert treatment effect estimate from log-OR to OR scale
tmp <- as.data.frame(pool_obj_prop)
tmp$OR <- exp(tmp$est); tmp$OR.lci <- exp(tmp$lci); tmp$OR.uci <- exp(tmp$uci)
tmp[,c("parameter","OR","OR.lci","OR.uci")]
```
...
dat_delta$delta <- dat_delta$is_missing*5

# Repeat the analyses with the delta-adjusted values (provided via argument delta) and pool results
ana_delta <- analyse(impute_obj_CIR, delta = dat_delta, vars = vars_an)
pool(ana_delta)
```
...
tipp_frame <- expand.grid(delta_control=seq(-5,15,by=1), delta_intervention=seq(-5,15,by=1))
tipp_frame$pval_6 <- tipp_frame$trt_effect_6 <- NA

for (i in (1:nrow(tipp_frame))){
  # Create delta-adjustment 
  dat_delta$delta <- (dat_delta$group=="Control")*(dat_delta$is_missing)*(tipp_frame$delta_control[i])+
                     (dat_delta$group=="Intervention")*(dat_delta$is_missing)*(tipp_frame$delta_intervention[i])
  # Analyse (using simplified analysis function) & pool 
  ana_delta <- analyse(impute_obj_CIR, delta = dat_delta, vars = vars_an, fun = compare_change_lastvisit)
  pool_delta <- as.data.frame(pool(ana_delta))
  tipp_frame$trt_effect_6[i] <- pool_delta[["est"]]
  tipp_frame$pval_6[i] <- pool_delta[["pval"]]
}

# Show delta values which lead to non-significant analysis results
subset(tipp_frame,pval_6>=0.05)


# Plot results
tipp_frame$pval <- cut(tipp_frame$pval,c(0,0.001,0.01,0.05,0.2,1),right=F,
                       labels=c("<0.001","0.001 - <0.01", "0.01- <0.05", "0.05 - <0.20",">= 0.20"))

ggplot(tipp_frame, aes(delta_control, delta_intervention, fill = pval)) +
    geom_raster()+scale_fill_manual(values=c("darkgreen","lightgreen","lightyellow","orange","red"))
```

According to this analysis, the significant test result from the primary analysis under CIR could only be tipped to a non-significant result for rather extreme delta-adjustments. 
