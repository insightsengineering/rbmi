...
If more than one chain is being run, the chains are currently initialized at `"random"` values as per `rstan` default. This and other expert options for the MCMC sampling can be defined via the `control` argument to `method_bayes`, which is simplified with the corresponding `control_bayes()` function call.
Note that for consistency with `rstan`, the burn-in can be customized via the `warmup` argument, and the burn-between can be customized via the `thin` argument.
