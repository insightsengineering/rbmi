...
However, since decisions are generally based not just on the treatment effect estimate but also on the p-value, it is important to calculate the corresponding MCSE. No closed-form expression exists for the MCSE of the p-value, but as argued by @Royston2009, the jackknife method could be used to estimate the MCSE. Let $\theta$ be the parameter of interest (e.g. the p-value) and let 
$\hat{\theta}_{(-m)}$ be the jackknife parameter estimate, obtained from the $M-1$ imputed datasets after removing the $m$-th imputed dataset. Also let $\overline{\theta}_{(\cdot)} = \sum_{m = 1}^{M} \hat{\theta}_{(-m)} / M$ be the average of the jackknife parameter estimates. The MCSE of the overall estimate $\hat{\theta}$ is then given by:
