...
Alternatively, the user can specify to use an LKJ prior for the unstructured correlation matrix, combined with independent scaled inverse chi-squared priors for the variances. The LKJ prior has density proportional to 1, i.e. it is a uniform prior on the space of correlation matrices. For a single variance $y$, we choose a single degree of freedom and use as scale $\sigma$ the estimated standard deviation from the frequentist REML fit. Then the prior density is given by:
$$
\text{ScaledInvChiSquare}(y \vert \nu,\sigma) =
\frac{(1 / 2)^{1/2}}      {\Gamma(1 / 2)} \, \sigma \,
y^{-(3/2)} \, \exp \! \left( \!    - \, \frac{1}{2} \, 
\sigma^2 \, \frac{1}{y} \right) 
 (\#eq:scaledInvChiSquare)
$$ 

#### Autoregressive order 1 covariance structure

For the time-homogeneous version of the AR(1) covariance structure, we use a flat prior on the correlation parameter $\rho$ and a scaled inverse chi-squared prior for the variance parameter $\sigma^2$, as described in \@ref{eq:scaledInvChiSquare}.
