...
For the time-homogeneous version of the AR(1) covariance structure, we use a uniform prior on the correlation parameter $\rho$ and a scaled inverse chi-square prior for the variance parameter $\sigma^2$, as described above.
...
For the compound symmetry covariance structure, we use a uniform prior on the correlation parameter $\rho$ in the allowed range $(-1/(J-1), 1)$. This is because for $\rho$ smaller than the lower boundary, the correlation matrix would not be positive definite any longer.
