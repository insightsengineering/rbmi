...
For further explanations, we also cite the end of section 2.4 of the conditional mean imputation paper (@Wolbers2021):
...
Yes. The default `ancova()` function supports two or more treatment groups directly, so no
...
For more than two groups the `ref` / `alt` naming scheme is extended with `alt2`, `alt3`,
etc. (based on the factor levels of `vars$group`). By default a treatment effect is
estimated for every non-reference group versus the reference group; the set of contrasts
can be customised via the `group_contrasts` argument of `set_vars()`. Custom contrasts
must be named, and the names become the output parameter names. For example, with three
arms `"Placebo"`, `"Low"` and `"High"` you can request a bespoke set of pairwise
contrasts:
...
anaObj <- rbmi::analyse(imputeObj, vars = vars)
```

Contrasts are not restricted to pairwise comparisons. A contrast can also be given as a
named numeric weight vector over the group levels (which must sum to zero), for example to
compare the average of the two active arms against `"Placebo"`. The list name is used as
the parameter name and is carried through to the `contrast_label` column of the `pool()`
output:
...
Please see the documentation for `ancova()` and `set_vars()` for more details.
