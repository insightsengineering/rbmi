...
The results from `ancova` are collapsed into a single list suffixed by the visit name.
The names of this list contains "ref" and "alt" to refer to the two treatment arms. Please note that "ref" refers to the first level of `vars$group` which not necessarily
coincides with the control arm. In this case, since `levels(dat[[vars$group]]) = c("DRUG", PLACEBO`), the results associated with "ref" correspond to the intervention arm, while those associated with "alt" correspond to the control arm.
