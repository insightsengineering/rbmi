...
#'
#' @param scale The scale of results returned by `as.data.frame.pool()` or
#' displayed by `print.pool()`. Must be `"auto"` (the default), `"original"`,
#' or `"transformed"`. `"auto"` uses transformed results when available and
#' original results otherwise.
#'
#' @param ... not used.
#'
#' @details
...
pool <- function(
    ...
    if (!is.null(results$transform)) {
        ret$transformed_pars <- transform_pooled_pars(pars, results$transform)
...
#' Transform pooled parameters for reporting
#'
#' @param pars A named list of pooled parameters.
#' @param transform A validated transformation specification.
#' @return A named list of transformed pooled parameters.
#' @keywords internal
#' @noRd
transform_pooled_pars <- function(pars, transform) {
    lapply(pars, transform_pooled_par, transform = transform)
}


#' Transform one pooled parameter for reporting
#'
#' @param par A pooled parameter.
#' @param transform A validated transformation specification.
#' @return A transformed pooled parameter.
#' @keywords internal
#' @noRd
transform_pooled_par <- function(par, transform) {
    transformed_est <- apply_pool_transform(
        transform$transform,
        par$est,
        "transform"
    )
    transformed_ci <- apply_pool_transform(
        transform$transform,
        par$ci,
        "transform"
    )
...
#' Apply and validate a pooled-result transformation function
#'
#' @param fun A transformation function.
#' @param x Numeric values to transform.
#' @param name The name of the transformation function, used in error messages.
#' @return The transformed numeric values.
#' @keywords internal
#' @noRd
apply_pool_transform <- function(fun, x, name) {
    value <- fun(x)
    assert_that(
        is.numeric(value),
        length(value) == length(x),
        msg = sprintf(
            "`transform$%s` must return a numeric vector of the same length as its input",
            name
        )
    )
    value
...
#' @keywords internal
as_data_frame_internal <- function(x, pars = x$pars) {
    assert_that(
        has_class(x, "pool") ||
            has_class(x, "mcse"),
    ...
    data.frame(
        ...
        est = vapply(pars, function(x) x$est, numeric(1)),
        se = vapply(pars, function(x) x$se, numeric(1)),
        lci = vapply(pars, function(x) x$ci[[1]], numeric(1)),
        uci = vapply(pars, function(x) x$ci[[2]], numeric(1)),
        pval = vapply(pars, function(x) x$pvalue, numeric(1)),
...
print.pool <- function(
    ...
    results_header <- if (scale == "original") {
        "Results:"
    } else {
        "Results (transformation applied; original-scale results are in `$pars`):"
    }
    results_df <- as.data.frame(x, scale = scale)
    ...
    string <- c(
        ...
        "",
        results_header,
        as_ascii_table(results_df, pcol = "pval"),
        ""
    )
