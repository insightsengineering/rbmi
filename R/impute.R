...
#'
#' @param references a named vector. Identifies the references to be used when generating
#' the imputed values. Should be of the form `c("Group1" = "Reference1", "Group2" = "Reference2")`.
#'
...
#' @details
#'
#' `impute()` works by taking the model parameter estimates, as generated by [draws()], and using
#' them to calculate the distribution of the outcome variable separately for each subject by
...
#' from the augmented imputation distribution after conditioning on the observed outcome values
#' with 1 dataset being generated per set of parameter estimates provided by [draws()].
#'
#' The exact manner in how missing values are imputed from this augmented distribution depends
#' on the method object that was provided to [draws()], in particular:
#'
#' - Bayes & Approximate Bayes: each imputed dataset consists of 1 row per subject & visit
...
#'
#' - Conditional Mean: each imputed dataset consists of 1 row per subject & visit from the
#' bootstrapped or Jackknife dataset that was used to generate the corresponding parameter
#' estimates in [draws()]. Missing values are imputed by using the mean of the augmented
#' outcome distribution. Please note that the first imputed dataset is the original
#' data i.e. 1 row per subject per visit (that is to say that it is not a bootstrap or jackknife
#' sample of the data)
#'
#' The `update_strategy` argument can be used to update the imputation strategy that was
#' originally set via
#' the `data_ice` option in [draws()]. The `update_strategy` dataframe must contain two columns,
#' one for the subject ID and another for the imputation strategy, whose names are the same as
#' those defined in `vars` argument as specified in the call to [draws()]. Please note that this
#' argument only allows you to update the imputation strategy, that is you cannot update the ICE
#' visit.
#' If you do need to change the ICE visit you will need to re-run [draws()]. A key limitation
...
#' if you try to switch them from MAR to a non-MAR strategy. In contrast, switching from
#' a non-MAR to MAR strategy, whilst valid, will raise a warning as not all usable data
...
#'
#' @param strategies A named list of functions. Defines the imputation functions to be used.
#' The names of of the list should mirror the values specified in `method` column of `data_ice`.
#' Default = `getStrategies()`. See [getStrategies()] for more details.
#'
...
#' @param beta List of model beta coefficients 1 for each sample i.e.
...
#'
#' This function creates the the object that is returned from [impute()]. Essentially
