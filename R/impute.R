...
#' of the list should mirror the values specified in `method` column of `data_ice`.
...
#' based upon the date of the intercurrent event (ICE), the selected strategy and the chosen reference group. Each imputed dataset is
#' then constructed based upon the method as specified in the call to [draws()] with 1 dataset being generated per
...
#' using the ML parameter estimates of the
...
#' a visit column has no effect here. If you wish to change the ICE visit you will need to re-run `draws`.
...
#'
#' @param beta A list of beta coefficients for each sample, i.e. `beta[[1]]` is the set of beta coefficients
#' for the first sample.
#'
#' @param sigma A list of the sigma coefficients for each sample split by group i.e. `sigma[[1]][["A"]]`
#' would give the sigma coefficients for group A for the first sample.
#'
