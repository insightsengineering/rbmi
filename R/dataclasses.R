...
#' Create a sample for a count-outcome imputation model
#'
...
sample_single_count <- function(ids, beta, phi) {
    ...
    class(x) <- c("sample_single_count", "sample_single", "list")
    validate(x)
    x
}
...
#' @export
validate.sample_single_count <- function(x, ...) {
    assert_that(
        identical(x$failed, FALSE),
        is.character(x$ids),
        length(x$ids) > 1,
        is.character(x$ids_samp),
        identical(x$ids_samp, x$ids),
        is.numeric(x$beta),
        length(x$beta) > 0,
