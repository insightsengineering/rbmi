...
prepare_prior_params <- function(
    ...
    if (covariance == "us") {
        ...
        stan_data$rho_par <- adjust_dimensions(same_cov, mmrm_initial$rho)
    } else if (covariance %in% c("ad", "toep")) {
        stan_data$sd_par <- adjust_dimensions(same_cov, mmrm_initial$sd)
        stan_data$rhos_par <- adjust_dimensions(same_cov, mmrm_initial$rhos)
    } else if (covariance %in% c("adh", "toeph")) {
        stan_data$sds_par <- adjust_dimensions(same_cov, mmrm_initial$sds)
        stan_data$rhos_par <- adjust_dimensions(same_cov, mmrm_initial$rhos)
    } else {
...
#'
#' @param outcome_type A character string indicating the type of outcome variable.
#' @param designmat The design matrix of the fixed effects.
#' @param outcome The response variable. Must be numeric.
#' @param outcome_unscaled The unscaled response variable. Must be numeric. If
...
#'   `visit` is used instead.
#' @param duration Numeric vector containing the duration variable. `NULL`
#'   if `period` is not used.
#' @param method A `method` object as generated by [method_bayes()].
#' @param scaler A `scaler` object as generated by [scalerConstructor()]. If
#'   omitted, one is created from `outcome` and `designmat`.
...
fit_mcmc <- function(
    ...
    # `fit_mcmc()` is also used directly in tests and by downstream code.  The
    # main `draws.bayes()` path supplies already scaled inputs and a scaler for
    # converting the posterior draws back to the original scale.  Preserve the
    # former direct-call interface by preparing those inputs when no scaler is
    # supplied.
    if (is.null(scaler)) {
        outcome_unscaled <- outcome
        model_df <- cbind(outcome, designmat)
        scaler <- scalerConstructor$new(model_df)
        model_df <- scaler$scale(model_df)
        outcome <- model_df[, 1, drop = TRUE]
        designmat <- model_df[, -1, drop = FALSE]
    }

    if (is.null(outcome_unscaled)) {
        outcome_unscaled <- outcome
    }
...
get_ESS <- function(stan_fit) {
    ...
    return(rstan::summary(
        stan_fit,
        pars = setdiff(stan_fit@sim$pars_oi, "lp__")
    )$summary[, "n_eff"])
}
...
#' Prepare input data to run the Stan model for continuous outcome
#'
#' @description
#' Prepare input data to run the Stan model for continuous outcome.
...
prepare_stan_data <- function(ddat, subjid, visit, outcome, group) {
    ...
    ddat <- remove_if_all_missing(ddat, timevar = "visit")
...
#' @param timevar the name of the time variable (e.g. `visit` or `period`) to use
remove_if_all_missing <- function(dat, timevar = "visit") {
    assert_that(timevar %in% names(dat))
    n_timepoints <- length(unique(dat[[timevar]]))
    n_miss <- tapply(dat$outcome, dat$subjid, function(x) sum(is.na(x)))
    remove_me <- Filter(function(x) x == n_timepoints, n_miss)
    remove_me_pt <- names(remove_me)
    dat[!dat$subjid %in% remove_me_pt, ]
}


#' Validate a `stan_data_continuous` object
#'
#' @param x A `stan_data_continuous` object.
#' @param ... Not used.
#'
#' @export
validate.stan_data_continuous <- function(x, ...) {
    assert_that(
        x$N == nrow(x$Q),
        x$P == ncol(x$Q),
        ...
        msg = "Invalid Stan Data Object for Continuous Outcome"
    )
}
