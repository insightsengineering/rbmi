...
#' @param ncores A single numeric specifying the number of cores to parallelise the frequentist
#' MMRM fitting over. See details. (Default = 1).
#'
#' @details
#'
...
draws.condmean <- function(data, data_ice = NULL, vars, method, ncores = 1) {
    ...
    if (method$type == "bootstrap") {
        ...
        x <- get_jackknife_draws(longdata, method, ncores = ncores)
    } else {
...
#' @param stack A R6 stack instance. Ignore this is only used for unit testing purposes
#'
#' @details
...
get_bootstrap_draws <- function(
    ...
    n_target_samples <- ife(first_sample_orig, method$n_samples + 1, method$n_samples)
    samples <- list()
    n_failed_samples <- 0
    ...
    # To avoid issues of non-random sampling across multiple cores we generate
    # the random ids in advance on the main processes
    #
    # If using multiple cores we produce `ncore` samples per iteration and thus
    # the maximium number of samples we can generate is potentially
    # greater than `n_target_samples` thus we need to create more ids as to not
    # run out (these will be pruned later depending on failures)
    max_samples <- ceiling((n_target_samples + failure_limit) / ncores) * ncores
    print(max_samples)
    stack$add(replicate(
        n = max_samples,
        expr = {
...
browser()
    while (length(samples) < n_target_samples) {
        ids_boot <- stack$pop(ncores)
        new_samples <- mmrm_sample(ids_boot)
        new_samples_keep <- Filter(function(x) x$failed == FALSE, new_samples)
        n_failed_samples <- n_failed_samples + length(new_samples) - length(new_samples_keep)
    ...
    # If using multiple cores it is possible for us to produce more samples than was required
    # Here we limit the number of samples generated down to the ammount needed
    samples <- samples[seq_len(n_target_samples)]
...
#'
#' @inherit as_draws return
get_jackknife_draws <- function(longdata, method, ncores = 1, stack = Stack$new()) {
    ...
    number_of_iterations <- ceiling(length(ids) / ncores)
    stack$add(
    ...
    for (iteration in seq_len(number_of_iterations)) {
        ids_jack <- stack$pop(ncores)
        new_samples <- mmrm_sample(ids_jack)
        is_sample_failure <- vapply(new_samples, function(x) x$failed, logical(1))
        sample_failure_index <- which(is_sample_failure)
        id_failure_index <- sample_failure_index * iteration
        if (length(id_failure_index)) {
            if (!is.null(cl)) parallel::stopCluster(cl)
            # Only show id of first failure as the error message may otherwise
            # confuse users into thinking the jackknife is incorrectly removing
            # multiple subjects at once
            stop(
                sprintf(
                    "MMRM failed to fit to the data after removing subject %s",
                    id_failure_index[1]
                )
            )
        }
        samples <- append(samples, new_samples)
    }
