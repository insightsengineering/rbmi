...
#' according to the given multiple imputation methodology. The models are different depending
#' on the outcome type that is used.
#'
#' For continuous outcomes, according to the user's method specification, it returns either draws from the posterior
#' distribution of the model parameters as required for Bayesian multiple imputation or frequentist parameter estimates from the
...
#' For this reason, any observed continuous outcome data after ICEs, for which reference-based imputation methods are
#' specified, are removed and considered as missing for the purpose of estimating the imputation model, and for
#' this purpose only. The imputation model for continuous outcomes is a mixed model for repeated measures (MMRM)
#' that is valid under a missing-at-random (MAR) assumption.
...
#'
#' For count outcomes, a Bayesian joint negative multinomial model is fitted to the event counts
#' across periods, accounting for each period's duration. Posterior draws of the regression
#' coefficients and dispersion parameter are returned for use in the imputation step.
#' Further details about the model are available in the statistical specifications for count
#' outcomes vignette.
#'
#' @name draws
...
#' @param outcome A single `character` specifying the outcome type. See details.
...
#' - `strata`: for continuous outcomes, covariates used as stratification variables in the bootstrap sampling.
...
#' - `period`: for count outcomes, name of the column in `data` which contains the period variable.
#' - `duration`: for count outcomes, name of the column in `data` which contains the duration variable.
#'
...
#' )
#'
#' # Derive data_ice: the first visit with a missing outcome for each patient is
#' # imputed under the Jump To Reference (JR) strategy. Subject 3618 has only an
#' # intermittent missing value and is removed (imputed under MAR by default).
...
#' )
#'
#' # Conditional mean imputation with jackknife resampling (no rstan required)
#' \donttest{
#' drawObj <- draws(
#'     data = dat,
#'     data_ice = dat_ice,
...
#' drawObj
#'
#' # Bayesian imputation instead requires the rstan package
...
draws.approxbayes <- function(
    ...
    assert_that(
        outcome == "continuous",
        msg = "Only continuous outcomes are supported for approximate Bayesian MI"
    )
...
draws.condmean <- function(
    ...
    assert_that(
        outcome == "continuous",
        msg = "Only continuous outcomes are supported for conditional mean MI"
    )
    longdata <- longDataConstructor$new(data, vars)
    longdata$set_strategies(data_ice)
...
draws.bmlmi <- function(
    ...
    assert_that(
        outcome == "continuous",
        msg = "Only continuous outcomes are supported for BMLMI"
    )
    longdata <- longDataConstructor$new(data, vars)
    longdata$set_strategies(data_ice)
...
get_draws_mle <- function(
    ...
    cl <- make_rbmi_cluster(
        ncores,
        objects = list("longdata" = longdata, "method" = method)
    )
...
get_mmrm_sample <- function(ids, longdata, method) {
    vars <- longdata$vars
    dat <- longdata$get_data(ids, mnar.rm = TRUE, na.rm = TRUE)
