...
#' @param ncores Deprecated. Use `future::plan()` instead. Please see `vignette("TODO")` for more
#' details.
...
#'
#' @export
draws <- function(data, data_ice = NULL, vars, method, ncores = NULL, quiet = FALSE) {
...
#' @rdname draws
#' @export
draws.approxbayes <- function(data, data_ice = NULL, vars, method, ncores = NULL, quiet = FALSE) {
...
#' @rdname draws
#' @export
draws.condmean <- function(data, data_ice = NULL, vars, method, ncores = NULL, quiet = FALSE) {
    longdata <- longDataConstructor$new(data, vars)
    longdata$set_strategies(data_ice)
...
#' @rdname draws
#' @export
draws.bmlmi <- function(data, data_ice = NULL, vars, method, ncores = NULL, quiet = FALSE) {
    longdata <- longDataConstructor$new(data, vars)
    longdata$set_strategies(data_ice)
...
get_draws_mle <- function(
    ...
    assert_that(
        is.null(ncores),
        msg = paste(
            "The `ncores` arguement has been deprecated, please use `future::plan()` instead.",
            "See `vignette(TODO)` for more details."
        )
    )
    ...
    if (!quiet) {
        cat(
            sprintf(
                "\nEstimated running time for sequential evaluation is %s seconds\n\n",
                round(time_taken[[3]] * n_target_samples)
            )
        )
    }
    ...
    #
    # A trade-off is required here, the larger the batch size the more performance
    # we get from not having to repeatedly copy data to sub-processes.
    # However this also means more computation is done before checking for failures
    # ideally we want to exit quickly in the case of failures to not waste computaiton.
    # If running sequentially then might as well check for failure after each iteration
    #
    n_samples_per_cycle <- if (future::nbrOfWorkers() > 1) {
        4 * future::nbrOfWorkers()
    } else {
        1
    }
    ...
    while (length(samples) < n_target_samples) {
        ids <- sample_stack$pop(min(n_samples_per_cycle, n_target_samples - length(samples)))
        new_samples <- future.apply::future_lapply(
            ids,
            get_mmrm_sample,
            future.seed = TRUE,
            longdata = longdata,
