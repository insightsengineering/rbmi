...
#' MMRM fitting over. See details TODO. (Default = 1).
#'
#' @details
#'
...
draws.approxbayes <- function(data, data_ice = NULL, vars, method, ncores = 1) {
    ...
    x <- get_draws_mle(
        longdata = longdata, 
        method = method,
        sample_stack = get_sample_stack("bootstrap", longdata, method),
        use_samp_ids = FALSE,
...
draws.condmean <- function(data, data_ice = NULL, vars, method, ncores = 1) {
    ...
    sample_opts <- list(
        ...
        sample_stack = get_sample_stack(method$type, longdata, method)
    )
...
# TODO
get_sample_stack <- function(type, longdata, method, stack = Stack$new()) {
    if(type == "bootstrap"){
