...
- Use pragmatic default priors, based on the initial frequentist model fit, for the covariance model parameters, with the possibility to add multiple prior choices
  ...
  - Note that for now we don't allow a specification of the `eta` parameter of the LKJ prior, this could be added later in a future update if needed, and would then be an expert option inside `control_bayes()`. Here we just set `eta = 1` for the LKJ prior, which is the default and most commonly used value.
...
- Note that we could use this update of the package to remove the previously deprecated arguments `burn_in` and `burn_between` from the `method_bayes` function. However, we still keep them, in order to allow for enough time to update existing code that uses these arguments. So they will only be removed in a future update.
...
We implement this via a `validate` method for the `method_bayes` return value (i.e. class `bayes`), such that it can be rechecked flexibly further downstream. That said, we can also have a `validate` method for the `method` class in general, which checks more general properties of the method object.
...
### Covariance and prior code snippets

In a separate directory `stan/covariance_prior`, we will have the following files:
...
Each of these contain the Stan code snippets for the respective covariance structure and prior combination:

- `functions`: Potential functions that are needed for the covariance structure (e.g. for the `ar1` covariance structure we will need a function to calculate the covariance matrix from the `rho` and `sd` parameters).
- `data`: What is the input data needed, that is the prior parameters.
- `parameters`: Definition of the covariance model parameters
- `transformed parameters`: mostly calculating Sigma from the covariance model parameters.
- `model`: priors on the covariance model parameters.

### `as_stan_fragments()` 

This function is adapted from the `jmpost` package. Since this package is also copyright by Roche, like `rbmi`, and Craig is the author of this functions, this is not a problem. Nevertheless it is acknowledged in an `@author` roxygen field.

It can parse the Stan code snippets from the `stan/covariance_prior` directory and return a list of Stan code fragments.
These are then each concatenated to single strings, such that they can be used to parametrize the `MMRM.stan` file.

### `MMRM.stan`

This is now a templated Stan model file, which includes `jinjar` placeholders for the relevant code snippets based on the `covariance` and `prior_cov` arguments.

We note that the rendered Stan model file will not have perfect indentation, but this is not a problem for the Stan compiler, and it is more important that the code is readable in the source file.
...
This function locates both the covariance/prior Stan fragment file as well as the master template file `MMRM.stan`. These are then combined correctly. It copies the resulting full Stan code file to the cache directory, and then creates the `rstan` model object from it and returns it.

### `get_session_hash`

This function can stay as it is currently, because the covariance and prior strings will be added to the Stan file name. 

### `clear_model_cache`

This function is updated with a `keep` argument, such that files matching this string in their name will not be deleted from the cache directory. This allows different Stan files for different covariance structures and priors to be cached separately in the same session, which is identified by the hash string and pass to `keep`. Then only files from old sessions will be deleted, but no files from the current session.
