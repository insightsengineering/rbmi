...
create_data_ice <- function(data, imp_drug, imp_placebo) {
    ...
    data_ice <- data_NA %>%
        group_by(PATIENT) %>%
        summarise(
            "strategy" = ifelse(THERAPY[1] == "DRUG", imp_drug, imp_placebo),
...
data[, c("PATIENT", "VISIT", "THERAPY")] <- as.data.frame(lapply(
    data[, c("PATIENT", "VISIT", "THERAPY")],
    as.factor
))
data[, c("basval", "change")] <- as.data.frame(lapply(
    data[, c("basval", "change")],
    as.numeric
))
