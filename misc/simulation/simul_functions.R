...
simul_data <- function(n = 100, H0 = TRUE) {
    ...
    model_coef <- c(
        log(0.015 / 0.985),
        log(0.025 / 0.975) - log(0.015 / 0.985),
        1.5
    )
    d2 <- calc_discontinue_dropout(
        data = d1,
        model = model,
...
calc_discontinue_dropout <- function(data, model, model_coef, prob_dropout) {
    ...
    d_stop <- data %>%
        group_by(patnum) %>%
        summarise(
            trt_stop_visit = ifelse(
                any(stop_trt == 1),
                min(visit[stop_trt == 1]),
                Inf
            ),
            .groups = "keep"
        ) %>%
        ungroup() %>%
        mutate(
            dropout_visit = ifelse(
                sample(
                    c(TRUE, FALSE),
                    size = length(unique(data$patnum)),
                    replace = TRUE,
                    prob = c(prob_dropout, 1 - prob_dropout)
                ),
                trt_stop_visit,
                Inf
            )
    ...
    data <- data %>%
        ...
        mutate(
            time_off_trt = ifelse(
                trt_stop_visit <= max(visit),
                pmax(0, time - time[visit == trt_stop_visit]),
                0
            )
        )
    # return dataset
...
adjust_trajectories <- function(
    data,
    stop_slope_change2 = 0,
    stop_slope_change1 = 0
) {
    ...
    data <- data %>%
        ...
        mutate(
            y = x +
                control *
                    cumsum(
                        c(0, diff(time_off_trt)) * c(0, stop_slope_change2)
                    ) +
                intervention *
                    cumsum(c(0, diff(time_off_trt)) * c(0, stop_slope_change1))
        )
    return(data)
}
